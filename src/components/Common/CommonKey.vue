<template>
  <span class="d-flex flex-row justify-center align-center">
    ...
    <template v-for="(shortcut, sIdx) in parsedShortcut">
      ...
      <template v-for="(key, kIdx) in shortcut">
        <span v-if="kIdx > 0" :class="delimiterColor" :key="`key_delimiter_${sIdx}_${kIdx}`"><slot name="key-delimiter">&nbsp;+&nbsp;</slot></span>
        <span :class="`${color} ${noDecorate ? '' : `keyboard-decorate${$vuetify.theme.dark ? '-dark' : ''} px-6 py-2 mx-2`} keyboard-style`" :key="`${sIdx}_${kIdx}`">
          {{ key }}
        </span>
      </template>
    </template>
