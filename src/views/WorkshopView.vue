        ...
        >
          <div class="align-self-end">
            ...
            <v-tooltip bottom open-delay="1000">
              <template v-slot:activator="{ on, attrs }">
                ...
                >
                  <div style="max-width: 160px" class="text-truncate">
                    {{
                      `${editing[item.id] ? "* " : ""}${item.businessCategoryDisplay}`
                    }}
                  </div>
              ...
              {{ item.businessCategoryDisplay }}
            </v-tooltip>
          </div>
        </v-slide-item>
      </v-slide-group>
      <v-hover v-slot="{ hover }">
        <v-btn @click="newTab" class="mb-n3" small icon>
...
import WorkshopTab from "../components/Workshop/WorkshopTab.vue";
import { v4 as uuidv4 } from "uuid";
export default {
  components: { CommonProgress, CommonKey, WorkshopTab },
  mounted() {
    const savedSessions = window.store.loadSessions('workshop');
        ...
        },
        {
          ...
          handler: () => {
            this.sessions[this.tab] &&
              this.closeTab(this.sessions[this.tab]);
          },
          name: "关闭",
          shortcut: ["ctrl", "w"],
          invisible: true,
  ...
  methods: {
    newTab() {
      ...
      window.ipc.invoke("createSession", session).then((result) => {
        if (result) {
          console.log(result);
          session.name = session.businessCategoryDisplay || '未选择业务类型';
